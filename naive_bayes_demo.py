...
st.write("""
    ...
    We use a simple dataset of short text documents for this demonstration.
""")
...
for doc, category in zip(X_test, predictions):
...
st.write("""
    ### How it Works
    - **Text Preprocessing:** The documents are converted into a matrix of token counts (bag-of-words model).
    - **Model Training:** A Naive Bayes classifier is trained on the processed text data.
    - **Prediction:** The model predicts the category for new documents based on learned patterns.
    - **Accuracy:** The proportion of test documents that are correctly classified by the model.
""")
